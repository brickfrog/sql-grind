...
SELECT m.group_key,m.ref_id,m.partner_id,s.score FROM group_members m JOIN group_scores s USING(group_key) WHERE m.group_key IN (SELECT group_key FROM group_members GROUP BY group_key HAVING count(*)=1) ORDER BY group_key,ref_id,partner_id;
