...
partner_text AS (
 ...
 SELECT *,row_number() OVER(PARTITION BY ref_id ORDER BY date_points DESC,text_points DESC,category_points DESC,word_points DESC,group_key) AS pos,
 count(*) OVER(PARTITION BY ref_id,date_points,text_points,category_points,word_points) AS peers
 FROM (SELECT DISTINCT m.ref_id,e.* FROM group_members m JOIN group_evidence e USING(group_key)
 WHERE e.quantity_points=30 AND e.text_points=30 AND e.category_points=10 AND e.word_points=10)
), provisional AS (
 SELECT r.ref_id,s.group_key,
 CASE WHEN s.ref_id IS NULL THEN 'missing' WHEN s.peers>1 THEN 'ambiguous' WHEN s.date_points>=10 THEN 'high' ELSE 'probable' END AS confidence,
 CASE WHEN s.ref_id IS NULL THEN 0 WHEN s.date_points>=10 THEN 95 ELSE 80 END::BIGINT AS score
 FROM reference_events r LEFT JOIN supported s ON r.ref_id=s.ref_id AND s.pos=1
), conflicts AS (
 ...
 CASE WHEN c.group_key IS NOT NULL THEN 'ambiguous' ELSE p.confidence END AS confidence,p.score
 FROM provisional p LEFT JOIN conflicts c ON c.group_key=p.group_key
)
SELECT d.ref_id,
 coalesce((SELECT to_json(list(DISTINCT m.partner_id::VARCHAR ORDER BY m.partner_id::VARCHAR))::VARCHAR FROM group_members m WHERE m.group_key=d.group_key AND m.ref_id=d.ref_id),'[]') AS partner_ids,
 d.confidence,(d.score::DECIMAL(9,4)/100)::DECIMAL(5,4) AS score,
 CASE d.confidence WHEN 'high' THEN 'Exact group quantity and strong text/category/word evidence qualify; date evidence orders alternatives.' WHEN 'probable' THEN 'Exact group quantity and strong lexical evidence qualify despite missing date evidence.' WHEN 'ambiguous' THEN 'Competing groups have equal or insufficiently separated evidence, or reuse a member.' ELSE 'No candidate group supplies sufficient evidence.' END AS explanation
FROM decisions d ORDER BY d.ref_id;
