...
partner_text AS (
 ...
 SELECT p.ref_id,CASE WHEN p.confidence IN ('high','probable') THEN p.group_key ELSE NULL END AS group_key,
 p.confidence AS confidence,p.score,p.margin
...
SELECT d.ref_id,
 ...
 CASE d.confidence WHEN 'high' THEN 'A unique strong group survives simultaneous member-conflict review.' WHEN 'probable' THEN 'A unique supported group has incomplete evidence; inspect its source facts.' WHEN 'ambiguous' THEN 'Competing groups have equal or insufficiently separated evidence, or reuse a member.' ELSE 'No candidate group supplies sufficient evidence.' END AS explanation
