...
Immutable, hash-verified assets are served cache-first. An already-controlled profile picks up a new deployed version within one revalidation window of a worker start-up: until then the superseded engine keeps serving, and past it both caches are renamed to the new version and the previous pair is deleted. That upgrade is exercised on a profile the worker already controls, not only by planting a foreign cache name. The document and the built application assets are served network-first with a cache fallback, so a deployment is never masked by a superseded copy. Cache names carry the bundle version and configuration hash, and caches that do not match the current version are deleted.
