...
| D04a | Katas, play icon | Opens “Katas — repetition drills”. It lists every authored drill pattern with its due count, retained count, and the reason the shape is worth practicing. The icon carries the number of drills due, and the status bar repeats it, so the schedule is visible before the surface is opened. Drills are graded at run time against an authored reference and never award or revoke challenge completion. |
...
| M04 | Query  | Execute, Parse, Cancel, Show Plan, Compare with Reference, Submit, Hint, Query History, Reset Challenge SQL. Hint reveals the next hint, or reopens the hints once the challenge is complete, where the remaining levels can still be revealed because credit is already recorded. Reset restores authored starter SQL after explicit acceptance. |
...
| W04  | SQL editor                                                  | Supports text editing, selection, completion, search, and undo through CodeMirror. A named editor exposes its language and challenge. Completion offers the columns of the tables named in the statement being written — after `FROM`, `JOIN`, `UPDATE`, `INTO` or `TABLE` — ranked above the dialect keywords that share those prefixes, with each column naming the table or tables that hold it. Scope is the statement, so a table named in a neighbouring statement contributes nothing. Qualified and aliased references remain the dialect's own completion. |
...
| W16  | Hint and remaining count                                    | Reveals the next authored hint. Three levels exist per challenge. Revealed hints remain readable without another attempt or penalty. A completed challenge reads Review hints, or Read hints where none were revealed, and can still reveal the remaining levels: completion is recorded and hints never affect it, so withholding the action left a labelled button opening an empty dialog. Revealing after completion never revokes it.  |
...
| W26  | Status bar cells                                            | Show real engine/version/thread state, elapsed interval, result count, and caret location. They are information, not inert buttons, except the last cell, which reports how many repetition drills are due and opens the drill surface. Its square reports the displayed message's outcome — neutral, working, success, or error — not merely engine readiness.       |
...
The section that explains the comparison also carries the control that starts it, and it is the only button for it. Patchouli's dock carried a duplicate; three entry points for one command, one of them in a commentary window and reporting under a tab named Execution plan, is not discoverability. The Query menu still lists the command, as it lists every command. The report has its own Comparison tab, which exists while a comparison is running or while its report is on screen, so starting one no longer switches to a tab reading “No plan collected” for the fifteen seconds the nine pairs take.
...
Exact submission scorecards show named outcomes for every grading variant, and a failed variant names what is wrong with a specific row rather than listing the ways a row can fail. A row the learner returned that is not expected is identified by its own ordering-key values; a row that differs from the expected row with those keys names the column and the value the learner returned; a row returned too many times reports both multiplicities. No expected scalar is ever printed: the column name, the learner's own value and the count are all information they already hold, while the expected value is the answer, and the curriculum reserves boundary guidance for the third hint. A row-count mismatch no longer stops the analysis, so “expected 9 rows, received 11” is followed by which returned row is the extra one.
...
The initial floating judge position leaves that footer and the status bar unobstructed. It previously ended level with the application window's lower edge, which put its whole action row over the engine readout at every window size. Saved user positions remain unchanged.
...
| J02 | Show style notes / Show what failed   | Names where it leads. With no failed submission it opens the current style and syntax notes and their source ranges. While a failed submission is current it reads Show what failed and moves focus to the per-variant scorecard, or to the message log when the run never graded: a button labelled diagnostics must not answer a failed submission with “Syntax OK, no warnings or style notes”. It never requests an LLM or claims unsupported evidence. |
...
The palette is defined once as a set of role tokens — chrome faces, bevel edges, fields, ink, accents, status colours and syntax colours — with light values taken from the palette the application shipped with, except where a shipped value failed measurement: the light caution and danger inks were darkened to #704500 and #800000, and the dark danger ink lightened to #e89b95, because all three were used as 10px bold status text. A dark theme supplies a second set of values for the same roles, and the Settings theme control offers System, Light and Dark. System is resolved through `matchMedia` and reacts to a change while the application is open, so the dark values live in one block rather than being duplicated under a media query. A theme is stored per device; a profile written before the theme existed has no such key, is loaded unchanged, and resolves to System.
...
Amber diagnostic text is measured rather than assumed. The style-count and warning inks in the Patchouli dock and the selected row count in the Object Explorer are asserted at or above 4.5:1 against their painted backgrounds in both themes, compositing through transparent layers to the first opaque surface; the light style count measured 1.01:1 before the change. Locked nodes and syntax colors have been swept against a 3:1 floor but are not asserted.
...
The three hint levels cover concept, query structure, and boundary guidance. Hint use does not reduce completion credit, and the hint count is stated as a fact rather than as a row in a scorecard: a definition list identical to the one holding Correctness read as a deduction while the surrounding text insisted hints cost nothing. Because credit is unaffected, hints remain revealable after a challenge is completed; a learner who finished without one is told the guidance is still there to read, instead of being offered a Review hints button that opens an empty dialog.
...
The banner names the subsystem that failed: authored content carries the `Content error:` prefix its loaders attach and reads "Content unavailable" with Retry content, while anything reaching the same banner from the engine or asset layer reads "SQL engine unavailable" with Reload SQL engine, a label distinct from the operation error bar's Retry engine because both banners can be open at once. Both retries reload the verified runtime; the distinction tells a reader whether to suspect the curriculum or the SQL engine.
...
Every one of the thirteen skills owns at least one pattern. Drills covered six of them while the curriculum grew to thirteen, so a learner who reached recursive CTEs or pivots — the material where recall matters most — had nothing to drill, and the retention surface quietly stopped applying to half the curriculum exactly where it was needed. The seven added patterns are `half-open-window` (basics), `except-versus-except-all` (sets), `utc-month-buckets` (dates), `recursive-descent` (rec), `long-to-wide` (pivot), `sargable-predicate` (plan) and `canonical-alias` (reconcile), bringing the set to fourteen patterns over fifty-one variations. Two skills need a note on what a drill can mean there: `plan` is graded on rows like every other drill, so its pattern drills the rewrite whose correctness rows can prove — the same result set with the filtered column left bare, which is what allows an index to be used — rather than asserting anything about a plan; and `reconcile` is graded by a rubric in its challenges, so its pattern drills the deterministic sub-skills the capstone is built from, canonicalising through an alias table without dropping unmatched rows and parsing text dates without failing the query, never the judgement call itself.
...
The drill surface lists every variation under the pattern that owns it, each with its own state: never drilled, due, retained, or the date it next comes due. A filter shows either the patterns with drills due or all of them, so a count of drills in the introduction is always reachable as individual drills below it. One control opens the drill that came due earliest, so practicing does not start by choosing a pattern from a list.

An open drill offers Check drill, which grades, and Execute, which runs the SQL against the drill's own dataset variant and shows the rows without grading anything: a drill that could only be graded gave a learner no way to look at what their own query returned. An ungraded run clears the previous verdict rather than leaving a pass or miss beside new rows. Show tables lists the drill dataset's tables and columns in the surface itself, because the surface covers the Object Explorer and a drill may name a dataset the workbench is not holding, which would make the explorer the wrong schema even if it were reachable.

Escape in the drill editor reaches the editor first: it dismisses an open completion popup or closes the editor's search panel, and only a keystroke the editor did not use closes the dialog. The dialog closed on every Escape, which discarded the drill and the SQL typed into it — easy to hit once a bare column prefix started completing. A selection collapses on the first Escape for the same reason, so the drill closes on the second; from any control that is not the editor, one Escape closes it.

The drill editor owns its caret, like the workbench and the labs. It is a controlled component that applies whatever selection it is handed, and a constant zero selection reset the caret to the start of the document after every keystroke, so typed text came out reversed.
...
Immutable, hash-verified assets are served cache-first. An already-controlled profile picks up a new deployed version within one revalidation window of a worker start-up: until then the superseded engine keeps serving, and past it both caches are renamed to the new version and the previous pair is deleted. That upgrade is exercised on a profile the worker already controls, not only by planting a foreign cache name. The suite sets the stored revalidation timestamp explicitly rather than waiting out the real minute, so both sides of the window are deterministic. The document and the built application assets are served network-first with a cache fallback, so a deployment is never masked by a superseded copy. Caches that do not match the current names are deleted.

Engine and bundle caches are named separately, because they change for different reasons. The engine cache carries the bundle version and the configuration hash. The bundle cache carries a digest of the shipped content’s own paths and hashes, read from the asset index the application already verifies a fetched file against. Both are required: the bundle version is hand-set and the configuration hash covers the engine configuration, so a content-only deploy — a reworded instruction, a corrected brief — changes neither. A cache named only after those two kept serving a superseded challenge against a freshly fetched index, so the application failed its own integrity check on that challenge indefinitely and the single affordance offered, Retry content, refetched the same stale bytes. Separate names also keep the cost honest: correcting a sentence refills kilobytes instead of re-downloading 47 MB of engine. A profile stranded by the previous single-cache layout recovers on its next visit with no manual purge, and the unreachable copies that stranded it are dropped from the engine cache once per worker start-up. A pointer written before the split carries no content digest, so the next start-up asks the network for one; where that fails the version half is still honoured, the engine cache is named as usual and the bundle is read from whichever cache already holds it, because discarding the pointer would leave an upgraded profile offline with no cache at all and 47 MB it could not name. While a profile is in that state nothing is deleted: the cache holding its content cannot be identified, so neither the purge nor the migration may touch it. The content-only deploy and that recovery are both exercised in the suite, which also asserts the bundle cache is keyed by a real digest rather than a fallback constant.
