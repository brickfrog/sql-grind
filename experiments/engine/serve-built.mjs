...
for(const line of (await readFile(resolve(root,'_headers'),'utf8')).split('\n')){
 ...
 if(!/^\s/.test(line)){
  const expression=line.trim().split('*').map(s=>s.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')).join('.*');
  rules.push({pattern:new RegExp('^'+expression+'$'),headers:{}});
 }else{
  const colon=line.indexOf(':');
  if(colon<0||!rules.length)throw Error('Invalid _headers line');
  rules.at(-1).headers[line.slice(0,colon).trim().toLowerCase()]=line.slice(colon+1).trim();
 }
}
const mime={'.html':'text/html; charset=utf-8','.js':'text/javascript; charset=utf-8','.wasm':'application/wasm','.parquet':'application/octet-stream'};
...
}).listen(4173,'127.0.0.1',()=>console.log('Built harness ready on http://127.0.0.1:4173'));
